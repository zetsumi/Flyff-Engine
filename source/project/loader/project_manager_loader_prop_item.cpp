...
bool fe::ProjectManager::loadPropItemJson(const std::string& fileName) noexcept
{
    ...
    for (auto& it : datas)
    {
        ...
        prop->maxRepair = reader.getNumber<std::int64_t, fe::type::json::value>(item["nMaxRepair"]);
        prop->handed = reader.getNumber<std::uint64_t, fe::type::json::value>(item["dwHanded"]);
        prop->flag = reader.getNumber<std::uint64_t, fe::type::json::value>(item["dwFlag"]);
...
bool fe::ProjectManager::loadPropItemXml(const std::string& fileName) noexcept
{
    ...
    auto fctLoaderItem = [&](fe::type::xml::node& item)
    {
        ...
        prop->maxRepair = reader.getNumber<std::int64_t>(item, "nMaxRepair");
        prop->handed = reader.getNumber<std::uint64_t>(item, "dwHanded");
        prop->flag = reader.getNumber<std::uint64_t>(item, "dwFlag");

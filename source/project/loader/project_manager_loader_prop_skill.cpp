...
bool fe::ProjectManager::loadPropSkillJson(const std::string& fileName) noexcept
{
    ...
    for (auto& it : datas)
    {
        ...
        prop->hardness = reader.getNumber<int64_t, fe::type::json::value>(item["nHardness"]);
        prop->handed = reader.getNumber<uint64_t, fe::type::json::value>(item["dwHanded"]);
        prop->heelH = reader.getNumber<uint64_t, fe::type::json::value>(item["dwHeelH"]);
...
bool fe::ProjectManager::loadPropSkillXml(const std::string& fileName) noexcept
{
    ...
    auto fctLoaderItem = [&](fe::type::xml::node& item)
    {
        ...
        prop->hardness = reader.getNumber<int64_t>(item, "nHardness");
        prop->handed = reader.getNumber<uint64_t>(item, "dwHanded");
        prop->heelH = reader.getNumber<uint64_t>(item, "dwHeelH");
